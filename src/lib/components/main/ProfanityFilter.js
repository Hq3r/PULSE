...
const DEFAULT_PROFANITY_LIST = [
  ...
  try {
    ...
    if (savedValue !== null) {
      initialEnabled = savedValue === 'true';
    }
  } catch (e) {
  ...
  class ProfanityFilter {
    ...
    constructor(customList = null, replacement = '***') {
      ...
      this.enabled = initialEnabled; // Initialize from localStorage
      
      // Create regex with word boundaries to match whole words only
      this.updateRegex();
      
      console.log(`ProfanityFilter initialized, enabled: ${this.enabled}`);
    }
    ...
    setEnabled(enabled) {
      ...
      console.log(`ProfanityFilter set to ${this.enabled ? 'enabled' : 'disabled'}`);
    }
